...
public class Calculator {
    public static double Calculate(string expression) {
        ArithmeticExpr endExpre = create_ExpressionTree(expression, 0, expression.Length - 1);
    ...
    static ArithmeticExpr create_ExpressionTree(string expression, int start, int end) {
        ...
        if (double.TryParse(expression.Substring(start, end - start + 1), out double num)) {
            ...
            if (tag < 0) {
                ...
                ptree = new NumberExpr(double.Parse(s[0]));
            } else {
                if (expression[tag] == '+')
